...
void generate_message(char *message, char *device_id)
{
    ...
    temperature, humidity, direction, intensity, height, device_id);
}

static int cmd_pub(int argc, char **argv)
{   
    char *dev_id = "meteo_station_1";
    while(true){
        emcute_topic_t t;
        unsigned flags = EMCUTE_QOS_0;

        char *topic = "devices/mqtt-broker/messages/events/";
        char message[200];
        generate_message(message, dev_id);
        ...
        if(dev_id[14] == '1') dev_id = "meteo_station_2";
        else dev_id = "meteo_station_1";
        xtimer_sleep(5);
    }
